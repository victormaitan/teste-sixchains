  ...
  <v-container class="mb-14" style="overflow-y: auto; height: 78vh">
    <v-card :ripple="false" :color="task.done ? 'grey-lighten-2' : 'accent'" class="mt-3 pa-2" rounded="xl"
      ...
      <v-row class="d-flex justify-space-between align-center">
        <v-col cols="2">
          <v-checkbox aria-label="Marcar ou desmarcar como concluído" v-model="task.done" :color="task.done ? 'grey-lighten-1' : ''" hide-details
            @click="editTask(task, 'check')" />
        </v-col>
        ...
        <v-col cols="2" class="d-flex justify-end">
          <v-dialog v-model="editDialog" max-width="600">
            <v-card prepend-icon="mdi-playlist-edit" title="Editar Tarefa" class="glass" rounded="xl">
              <v-card-text>
                <v-row dense>
                  <v-col cols="12">
                    <v-text-field placeholder="Descrição" aria-label="Editar descrição" required v-model="editedTask.description"></v-text-field>
                  </v-col>
                </v-row>
              </v-card-text>
          ...
          <v-btn class="mr-2" icon variant="tonal" aria-label="Excluir"  :color="task.done ? 'grey-lighten-1' : 'red'" density="comfortable"
